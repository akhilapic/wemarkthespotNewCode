...
class userController extends Controller
{
    ...
    public function community_reviews(Request $request)
    {
        ...
        if ($request->input()) {
            ...
            // $usreData = User::join('business_reviews', 'business_reviews.user_id', '=', 'users.id')
            //     ->where(['users.id'=> $user_id,'business_reviews.status'=>0])->orderBy('business_reviews.id', 'desc')
            //     ->get(
            //             [
            //                 'users.*', 
            ...
            $usreData = BusinessReviews::join('users', "users.id", "=", "business_reviews.user_id")
                                        ->where(array('business_reviews.user_id' => $user_id, 'business_reviews.business_id' => $business_id))
            ...
            if (isset($usreData)) {
                foreach ($usreData as $review) {
                    ...
                    $business_reviewlikeData = Businessreviewlikedislike::where('businessreview_id', $review->business_reviews_id)->get();

                    if (isset($business_reviewlikeData)) {
                        foreach ($business_reviewlikeData as $likedislike) {
                            if ($likedislike == 1) {
                                $like_status=1;
                                $like += 1;
                            } else {
                    ...
                    $review->total_like->like_status;
                    $review->total_like = $like;
                    $review->total_dislike = $dislike;
                }
            }
    ...
    public function community_reviews1(Request $request)
    {
        //Businessreviewlikedislike
        if ($request->input()) {
            $user_id = $request->user_id;
            $business_id = $request->business_id;
            // $users = BusinessReviews::where(array('user_id'=>$user_id,'business_id'=>$business_id))->get();
            // dd($users);
            // $usreData = User::join('business_reviews', 'business_reviews.user_id', '=', 'users.id')
            //     ->where('users.id', $user_id)->orderBy('business_reviews.id', 'desc')
            //     ->get(['users.*', 'business_reviews.ratting','business_reviews.business_id as business_id', 'business_reviews.id as business_reviews_id', 'business_reviews.review', 'business_reviews.image as business_review_image']);


            $usreData = BusinessReviews::join('users', "users.id", "=", "business_reviews.user_id")
                ->where(array('user_id' => $user_id, 'business_id' => $business_id))
                ->get(
                    [
                        'users.id as user_id,users.name as user_name,user.image as user_image',
                        'business_reviews.ratting',
                        'business_reviews.status',
                        'business_reviews.business_id as business_id',
                        'business_reviews.id as business_reviews_id',
            ...
            //dd($usreData);
            // $usreData = User::join('business_reviews', 'business_reviews.user_id', '=', 'users.id')
            //                ->where(['business_reviews.id'=> $user_id,'business_reviews.business_id'=>$bus,'business_reviews.status'=>0])->orderBy('business_reviews.id', 'desc')
            //                ->get(
            //                        [
            //                            'users.*', 
            //                            'business_reviews.ratting',
            ...
            //                     );
            //                dd($usreData);
            //   $usreData  = BusinessReviews::where(['user_id'=>$user_id,'business_id'=>$business_id])->get();
    ...
    public function gethotspots()
    {
        $hotspots = Hotspots::join('users', 'users.id', '=', 'hotspots.user_id')
            ...
            ->get(['users.name as person_name', 'users.image as user_image', 'business_users.name as business_user_name', 'hotspots.*']);
    ...
    public function BusinessFav(Request $request)
    {
        if ($request->input()) {
            ...
            if ($fav == 1) {
                ...
                if ($checked) {
                    $result = array("status" => true, 'message' => "Business added to favorites.");
                } else {
                    $result = array("status" => false, 'message' => "Business removed from favorites falils.");
                }
            } else {
    ...
    public function Businesslikedislike(Request $request)
    {
        ...
        if ($Validation->fails()) {
            ...
            $data = BuinessReports::create(array('user_id' => $request->user_id, 'business_id' => $request->business_id, 'businessreview_id' => $request->businessreview_id, 'likedislike' => $request->likedislike));
            if ($data) {
